...
export default function CharSelect({ onChangeState, onStart }: { onChangeState: (s: GameState) => void, onStart: (h: string, w: string) => void }) {
    ...
    const selectHero = (h: string) => { AudioManager.play('ui', 'select'); setHero(h); };
    const selectWeapon = (w: string) => { AudioManager.play('ui', 'select'); setWeapon(w); };
    const handleStart = () => { AudioManager.play('ui', 'select'); onStart(hero!, weapon!); };
    const handleBack = () => { AudioManager.play('ui', 'select'); onChangeState('TITLE'); };
    ...
    return (
        <section className="w-full h-full flex flex-col items-center justify-start gap-6 pointer-events-auto-override overflow-y-auto pb-20 pt-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 w-full justify-center px-4">
                <div onClick={() => selectHero('fugitivo')} className={`metal-frame border-4 p-1 w-full max-w-[280px] cursor-pointer transition-transform hover:scale-105 ${hero === 'fugitivo' ? 'glow-border border-primary' : 'border-surface-variant opacity-80'}`}>
                ...
                <div onClick={() => selectHero('escudeiro')} className={`metal-frame border-4 p-1 w-full max-w-[280px] cursor-pointer transition-transform hover:scale-105 ${hero === 'escudeiro' ? 'glow-border border-primary' : 'border-surface-variant opacity-80'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full justify-center px-4 max-w-4xl">
                {[
                    ...
                    <div key={w.id} onClick={() => selectWeapon(w.id)} className={`metal-frame border-2 p-2 cursor-pointer hover:opacity-100 ${weapon === w.id ? 'glow-border border-primary' : 'border-surface-variant opacity-80'}`}>
            ...
            <div className="flex gap-4 mt-2 w-full max-w-md">
                <button className="metal-button flex-1 font-headline-md py-3 uppercase" onClick={handleBack}>Voltar</button>
                {hero && weapon && <button className="metal-button flex-1 font-headline-md py-3 uppercase" onClick={handleStart}>Explorar Ruínas</button>}
            </div>
        </section>
    );
}
